...
const EventKanban = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (leaderMember?.status === 'approved' && ['core_team', 'lead', 'faculty_coordinator'].includes(leaderMember?.role)) {
  ...
  return (
    <div className="container" style={{ padding: '2rem 0', maxWidth: '1200px' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ margin: '0 0 0.5rem 0' }}>Kanban Board</h2>
        <p style={{ color: 'var(--primary-color)', margin: 0, fontWeight: 600 }}>{event?.name}</p>
      </div>

      <div style={{ display: 'flex', gap: '1.5rem', overflowX: 'auto', paddingBottom: '1rem' }}>
        {renderColumn('todo', 'To Do', '#3b82f6')}
        {renderColumn('in_progress', 'In Progress', '#f59e0b')}
